...
| **Stitch holes** | diagonal slits for hand sewing (leather/felt) — click two edges or guide lines and both get the **same number of holes at matching positions**; guides get holes along their whole length (including closed rings) · **bulk mode: `Shift`-click (or drag a box over) many edges/guide lines**, then Enter or "Stitch selected" runs holes along each in one step — contiguous edges of a piece stitch as **one continuous run along the mitred inset line** (Inset setting honoured around corners; the full outline becomes a single loop) · every run is **one object**: click any of its holes and "Delete stitch line" removes the whole run (Del still removes a single hole) — holes stay separate marks until export, where they become cut slits · spacing/slit length configurable, exported on `CUT` |
